...
2. `grep xxx`：从 `stdin` 中读入若干行数据，如果某行中包含xx，则输出该行；否则忽略该行。

    `grep -o -E '\w+' text.txt | uniq -c | sort -nr` ：使用 grep 命令从文件中提取单词

    -o 参数表示只输出匹配的部分，-E 参数表示使用正则表达式匹配单词，'\w+' 表示匹配一个或多个单词字符。

    `uniq` 命令用于去重并统计每个单词出现的次数，`-c` 参数表示统计出现次数。`sort` 命令再次使用，用于按照出现次数从大到小排序输出。
